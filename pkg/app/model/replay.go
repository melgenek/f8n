...
func NewReplay(persistedRequests []model.EtcdRequest) *EtcdReplay {
	...
	var events []model.PersistedEvent
...
func (r *EtcdReplay) EventsForWatch(watch model.WatchRequest) (events []model.PersistedEvent) {
...
func toWatchEvents(prevState *EtcdState, request model.EtcdRequest, response model.MaybeEtcdResponse) (events []model.PersistedEvent) {
	if response.Error != "" {
		return events
	}
