...
func ValidateAndReturnVisualize(lg *zap.Logger, cfg Config, reports []report.ClientReport, persistedRequests []model.EtcdRequest, timeout time.Duration) (result validate.RobustnessResult) {
	result.Assumptions = validate.ResultFromError(checkValidationAssumptions(reports))
