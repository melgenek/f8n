...
func validateLinearizableOperationsAndVisualize(lg *zap.Logger, operations []porcupine.Operation, timeout time.Duration) validate.LinearizationResult {
	lg.Info("Validating linearizable operations", zap.Duration("timeout", timeout))
	start := time.Now()
	check, info := porcupine.CheckOperationsVerbose(forkedModel.NonDeterministicModel, operations, timeout)
	result := validate.LinearizationResult{
		Info:  info,
		Model: forkedModel.NonDeterministicModel,
	}
	switch check {
	case porcupine.Ok:
...
func validateSerializableRead(lg *zap.Logger, replay *forkedModel.EtcdReplay, request model.EtcdRequest, response model.MaybeEtcdResponse) error {
	...
	// Kine does not support versions
	kvs := make([]model.KeyValue, 0, len(expectResp.Range.KVs))
	for _, kv := range expectResp.Range.KVs {
		kv.Version = 0
		kvs = append(kvs, kv)
	}
