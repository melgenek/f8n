#!/bin/bash
# copied from https://github.com/apple/foundationdb/blob/main/packaging/docker/fdb.bash
# Modified to run TLS
...
function create_cluster_file() {
    ...
    if [[ -n "$FDB_CLUSTER_FILE_CONTENTS" ]]; then
        ...
        echo "docker:docker@$coordinator_ip:$coordinator_port$tls_option" > "$FDB_CLUSTER_FILE"
    else
...
function create_server_environment() {
    ...
    if [[ -z $FDB_COORDINATOR && -z "$FDB_CLUSTER_FILE_CONTENTS" ]]; then
        FDB_CLUSTER_FILE_CONTENTS="docker:docker@$public_ip:$FDB_PORT$tls_option"
    fi
...
echo "Starting FDB server on $PUBLIC_IP:$FDB_PORT$tls_option"
fdbserver --listen-address 0.0.0.0:"$FDB_PORT$tls_option" --public-address "$PUBLIC_IP:$FDB_PORT$tls_option" \
