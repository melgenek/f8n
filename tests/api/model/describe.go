...
func describeEtcdResponse(request model.EtcdRequest, response model.MaybeEtcdResponse) string {
	if response.Error != "" {
	...
	case model.LeaseGrant, model.LeaseRevoke:
		return fmt.Sprintf("ok, rev: %d", response.Revision)
	case model.Compact, model.Defragment:
		return "ok"
	default:
...
func describeEtcdRequest(request model.EtcdRequest) string {
	switch request.Type {
	case model.Range:
...
func describeGuaranteedTxn(txn *model.TxnRequest) string {
...
func describeEtcdConditions(conds []model.EtcdCondition) string {
...
func describeEtcdOperations(ops []model.EtcdOperation) string {
...
func describeTxnResponse(request *model.TxnRequest, response *model.TxnResponse) string {
...
func describeEtcdOperation(op model.EtcdOperation) string {
	switch op.Type {
...
func describeRangeRequest(opts model.RangeOptions, revision int64) string {
	kwargs := []string{}
	if revision != 0 {
...
func describeEtcdOperationResponse(op model.EtcdOperation, resp model.EtcdOperationResult) string {
	switch op.Type {
	case model.RangeOperation:
...
func describeRangeResponse(request model.RangeOptions, response model.RangeResponse) string {
	if request.End != "" {
...
func describeValueOrHash(value model.ValueOrHash) string {
	if value.Hash != 0 {
