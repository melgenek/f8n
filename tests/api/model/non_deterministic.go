...
var NonDeterministicModel = porcupine.Model{
	...
	Step: func(st any, in any, out any) (bool, any) {
		return st.(nonDeterministicState).apply(in.(model.EtcdRequest), out.(model.MaybeEtcdResponse))
	},
	...
	DescribeOperation: func(in, out any) string {
		return fmt.Sprintf("%s -> %s", describeEtcdRequest(in.(model.EtcdRequest)), describeEtcdResponse(in.(model.EtcdRequest), out.(model.MaybeEtcdResponse)))
	},
...
func (states nonDeterministicState) apply(request model.EtcdRequest, response model.MaybeEtcdResponse) (bool, nonDeterministicState) {
...
func (states nonDeterministicState) applyFailedRequest(request model.EtcdRequest) nonDeterministicState {
...
func (states nonDeterministicState) applyPersistedRequest(request model.EtcdRequest) nonDeterministicState {
...
func (states nonDeterministicState) applyPersistedRequestWithRevision(request model.EtcdRequest, responseRevision int64) nonDeterministicState {
	newStates := make(nonDeterministicState, 0, len(states))
	for _, s := range states {
...
func (states nonDeterministicState) applyRequestWithResponse(request model.EtcdRequest, response model.EtcdResponse) nonDeterministicState {
	newStates := make(nonDeterministicState, 0, len(states))
	for _, s := range states {
		newState, modelResponse := s.Step(request)
